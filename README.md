...
### Cloudflare に依存しない選択肢

ギャラリーシェア機能は Cloudflare R2 / KV を使う [gallery-worker/](./gallery-worker/) と、
ファイルシステムだけで動く自前ホスト版 [gallery-server/](./gallery-server/) の
2 系統から選べます。

- **[gallery-worker/](./gallery-worker/)（Cloudflare）**: `wrangler deploy` 一発で
  立ち上がる。R2 ストレージ 10GB 無料 / Egress 無制限。普通はこちら推奨
- **[gallery-server/](./gallery-server/)（自前ホスト）**: Bun で動く 1 プロセス。
  R2 → ローカルファイル、KV → JSON ファイル。**自宅PC + Cloudflare Tunnel** や
  VPS など好きな環境で動かせる。詳細は [gallery-server/SETUP.md](./gallery-server/SETUP.md)
- **ギャラリー機能を使わない**: 写真整理・投稿管理・現像連携は外部依存なしで
